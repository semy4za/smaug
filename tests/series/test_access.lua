...
do
    ...
    local nan = 0/0
    local s_nan = Series.float64(3)
    s_nan:set(1, nil)    -- null
    s_nan:set(2, nan)    -- NaN (valor)
    s_nan:set(3, 5.0)    -- normal
    local f_nan = s_nan:fillna(99)
    check(f_nan:get(1) == 99, "fillna: null → 99")
    check(is_nan(f_nan:get(2)), "fillna: NaN permanece NaN (não é null, não preenche)")
    check(f_nan:get(3) == 5.0, "fillna: valor normal intacto")
    check(f_nan:is_null(2) == false, "fillna: posição NaN não era null")
    ...
    local df = DataSet.from_columns({
        { "a", {1.0, NA, 3.0}, "float64"},
        { "b", {NA, 20.0, 30.0}, "float64"},
    })
    -- fillna(valor) → todas as colunas
    local f_df = df:fillna(0)
    check(f_df:column("a"):get(2) == 0, "DS fillna: col a preenchida")
    check(f_df:column("b"):get(1) == 0, "DS fillna: col b preenchida")
    -- original intacto
    check(df:column("a"):is_null(2) == true, "DS fillna: original intacto")
    -- fillna({col=valor}) → por coluna; coluna omitida mantém nulos
    local f_df2 = df:fillna({a = -1})
    check(f_df2:column("a"):get(2) == -1, "DS fillna map: col a = -1")
    check(f_df2:column("b"):is_null(1) == true, "DS fillna map: col b omitida mantém null")
end
...
do
    ...
    local sb  = Series.new("int64", 1, "big"); sb:set(1, BIG)
...
do
    ...
    local s = Series.new("int64", 2, "big"); s:set_null(1); s:set(2, BIG)
    local r = s:fillna(0)
    ...
    local sv = Series.new("string", 2, "sv"); sv:set_null(1); sv:set_null(2)
    local rv = sv:fillna("")
    ...
    local sm = Series.new("string", 2, "sm"); sm:set_null(1); sm:set(2, "abc")
    local rm = sm:fillna("")
...
do
    ...
    local neg = Series.new("int64", 1, "neg"); neg:set(1, NEG)
    ...
    local big = Series.new("int64", 1, "big"); big:set(1, BIG)
    ...
    local m = Series.new("int64", 1, "min")
    ...
    local wn = Series.new("int64", 2, "wn"); wn:set_null(1); wn:set(2, 5)
...
do
    ...
    local wn = Series.new("int64", 3, "wn")
